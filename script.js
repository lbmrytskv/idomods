const FormaSintApp = (function () {
  ...
  // Logo and hero image click effect
  function initImageEffects() {
    const logoImg = document.querySelector('.logo');
    const heroImg = document.querySelector('.hero-img');
    
    // Logo effect
    const logoOriginalSrc = logoImg.src;
    const logoOriginalSrcset = logoImg.srcset;
    ...
    function handleLogoClick() {
      ...
      setTimeout(() => {
        ...
        logoImg.srcset = logoOriginalSrcset;
      }, CONSTANTS.LOGO_ANIMATION_DURATION);
    }

    // Hero image effect
    function handleHeroImageClick() {
      // Change to FLETTER.png on click
      const originalSrc = heroImg.src;
      const originalSrcset = heroImg.srcset;
      
      heroImg.src = "/assets/icons/FLETTER.webp";
      heroImg.srcset = ""; // Clear srcset for single image
      
      // Reset to original after animation duration
      setTimeout(() => {
        heroImg.src = originalSrc;
        heroImg.srcset = originalSrcset;
      }, CONSTANTS.LOGO_ANIMATION_DURATION);
    }

    // Add click event to hero image
    if (heroImg) {
      heroImg.addEventListener('click', handleHeroImageClick);
      heroImg.style.cursor = 'pointer'; // Make it clear it's clickable
    }

    return { handleLogoClick, handleHeroImageClick };
  }
  ...
  function handleNavigationClick(e, link) {
    ...
    if (link.classList.contains('logo-link')) {
      const { handleLogoClick } = initImageEffects();
      handleLogoClick();
    }
  ...
  // Popup functionality (without keyboard support)
  function initPopup() {
  ...
  function loadProducts() {
    fetch(`https://brandstestowy.smallhost.pl/api/random?pageNumber=${currentPage}&pageSize=${pageSize}`)
      ...
      .then((data) => {
        ...
        cachedProducts = data.data; // cache products
  ...
  function initCustomDropdown() {
    ...
    const customToggle = document.querySelector(".custom-select-toggle");
  ...
  function initResizeHandler() {
    window.addEventListener('resize', () => {
      ...
      resizeTimeout = setTimeout(() => {
        if (currentPage === 1 && cachedProducts) {
          renderProducts(cachedProducts); //  Use cached data on resize
        }
  ...
  return {
    init() {
      ...
      initResizeHandler();
      initNavigation();
      initImageEffects(); // Initialize image effects
